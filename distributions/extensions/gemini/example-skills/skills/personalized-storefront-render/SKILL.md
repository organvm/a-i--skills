...
description: Translate internal markdown artifacts (pitch decks, business plans, research, schemas) into per-persona client-facing storefront surfaces in the client's domain language with ELI5/TLDR layers. Substrate sits above Product Domain Engine (PDE) and 8-Strata Domain Ideal-Whole; invokes voice-enforcement against persona-specific voice_constitution. Triggers when the user asks for client-facing surfaces, persona-tuned translation, ELI5/TLDR projections of internal docs, or storefront-style client deliverables. Operates per-artifact via `audiences:` YAML frontmatter; per-repo via `storefront.config.yaml`; per-persona via `~/Documents/personas/{id}.lexicon.yaml`. Render is scaffold-and-curate (never auto-publish) to protect client trust.
---
